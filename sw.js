...
/* The exact app shell — same-origin files required for the app to load and
   run with zero network access. Keep this list in sync with what's actually
...
self.addEventListener('fetch', (event) => {
  ...
  if (url.origin === self.location.origin) {
    if (request.mode === 'navigate' || SHELL_ASSETS.some((a) => url.pathname.endsWith(a.replace('./', '/')))) {
      event.respondWith(
        staleWhileRevalidate(request, SHELL_CACHE).catch(() =>
          caches.match('./gym-meal-tracker-6_8.html')
        )
      );
      return;
    }
